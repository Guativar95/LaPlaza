...
export type UsePersonalInfoForm = {
  ...
  defaultValues?: Pick<
    DefaultValuesForm<PersonalInfoValues>,
...
export const usePersonalInfoForm = ({
  ...
  const otp = useApplicationOtp({ applicationId, initialVerificationStatus })
  const [showOtpModal, setShowOtpModal] = useState(false)
  const mounted = useRef(false)

  const [isLoading, setIsLoading] = useState(true)
  const [verificatedContactValues, setVerificatedContactValues] = useState({
  ...
  const handleSubmit = (values: PersonalInfoValues) => {
    let status = otp.verificationStatus
    if (status === OTPVerificationStatus.verified) {
      const { email, phoneNumber } = verificatedContactValues
      const verificatedValuesChanged = values.email !== email || values.phoneNumber !== phoneNumber

      if (verificatedValuesChanged) {
  ...
  useEffect(() => {
    ...
    onChangeVerificationStatus?.(otp.verificationStatus)

    if (personalInfo && otp.verificationStatus === OTPVerificationStatus.verified) {
      // set values verificated
      setVerificatedContactValues({
