...
export type DesactiveFromShowcaseModalProps = {
  show: boolean
  vehicleId: string
  onClose: () => void
  onSuccess?: () => void
...
export const DesactiveFromShowcaseModal: FC<DesactiveFromShowcaseModalProps> = ({
  show,
  onClose,
  vehicleId,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (status === 200) {
        onClose()
        onSuccess && onSuccess()
        toast.success('Vehículo en vitrina')
      }
    } catch (error) {
      console.error(error)
    ...
    <Modal show={show} size='lg' onClose={onClose}>
      ...
      <ModalBody>
        <p className='text-lg  text-primary-700 font-bold'>
          ¿Esta seguro que desea desactivar el vehículo en la vitrina?
        </p>
